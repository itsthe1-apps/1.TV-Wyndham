        ...
        case "KEY_RIGHT":
            ...
            if (top.DEFAULT_DIRECTION == "ltr") {
                ...
                if (correct_menu_id != 0) {
                    menuMainList.scrollDown(correct_menu_id+1);
                }
            }
            break;
        case "KEY_LEFT":
...
//Services Promotions
function servicesPromotionData(){

    var json_url = top.TICKER_MEDIA_URL+"en/format/json";
    top.kwUtils.kwXMLHttpRequest("GET", json_url, true, this, myFunctionServices);


}

function myFunctionServices(jsonString) {
    ...
    for (var i = 0; i < json.length; i++) {
      ...
      if (promo_type == "SERVICES") {
        promotions = json[i].ticker_promo_txt;
      }
    }
    servicesPromDisplayItem(promotions);
