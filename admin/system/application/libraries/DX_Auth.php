...
class DX_Auth
{
	...
	function _init()
	{
		...
		$this->update_view = $this->ci->config->item('DX_update_view');
	...
	*/
	...
	function _set_last_ip_and_last_login($user_id)
	{
		...
		if ( ! empty($data))
		{
			...
			// Update record
			//JK$this->ci->users->set_user($user_id, $data);
		}
	}
	...
	*/
	...
	{
		$this->ci->load->model('dx_auth/users', 'users');
		$Q = $this->ci->users->get_user_by_id($id);
		$data = array();
	...
	{
		return "<span class='error'>".$this->_auth_error."</span>";
	}
	...
	function register($username, $password, $contact_no, $email,$role,$staff_code)
	{		
		// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		$this->ci->load->model('dx_auth/user_temp', 'user_temp');
	...
	function edit_ip($id, $username, $password, $contact_no, $email, $role,$staff_code)
	{		
		// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		$this->ci->load->model('dx_auth/user_temp', 'user_temp');
		...
		//Check password field is ecrypted or not
		if(strlen($password)>20){
				$password_enc=$password;
			}else{
				$password_enc=crypt($this->_encode($password));
			}
		...
		//print_r($update_user);
		$update = $this->ci->users->set_user_ip($id,$update_user);
	...
	function captcha()
	{
		...
		$this->ci->load->plugin('dx_captcha');
		 ...
		 $captcha_folder='captcha'; //created by jk
		//$captcha_dir = 'captcha';
	...
	// Check add button permission for logged in user	
	function add_button($role_id, $current_uri)
	{
		if(!$this->is_admin())
		{
			$this->ci->db->select("tabs.add", FALSE);
			$this->ci->db->where('url',$current_uri);
			$Q = $this->ci->db->get('tabs');
			if ($Q->num_rows() > 0)
			{
			...
			$permission 	= $this->_get_role_data($role_id);
			$get_permission = $permission['permission']['uri'];
			
			$result = !empty($url) ? in_array($url."/",$get_permission) ? TRUE : FALSE : FALSE;
		}else{
			$result = TRUE;
		}
	...
	function edit_button($role_id, $current_uri)
	{
		if(!$this->is_admin())
		{
			$this->ci->db->select("tabs.edit", FALSE);
			$this->ci->db->where('url',$current_uri);
			$Q = $this->ci->db->get('tabs');
			if ($Q->num_rows() > 0)
			{
	...
	function delete_button($role_id, $current_uri)
	{
		if(!$this->is_admin())
		{
			$this->ci->db->select("tabs.delete", FALSE);
			$this->ci->db->where('url',$current_uri);
			$Q = $this->ci->db->get('tabs');
			if ($Q->num_rows() > 0)
			{
