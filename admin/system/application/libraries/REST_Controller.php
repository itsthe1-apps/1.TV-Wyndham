...
class REST_Controller extends Controller
{
     ...
     * Takes pure data and optionally a status code, then creates the response
     */
    function responseHTML($data = array(), $http_code = 200)
    {
   		if (empty($data))
    	{
    		$this->output->set_status_header(404);
		...
		//print $this->request->format;
        // If the format method exists, call and return the output in that format
        if (method_exists($this, '_format_html'))
        {
			
	    	// Set the correct format header
	    	$this->output->set_header('Content-type: '.$this->_supported_formats['html']);
			
			//$formatted_data = $this->{'_format_'.$this->request->format}($data);
        	//$formatted_data = $this->{'_format_html'}($data);
			//print_r($data);
        	$this->output->set_output($data);
        }
