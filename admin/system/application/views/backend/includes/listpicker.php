...
class listpicker extends controls
{
	...
	function listpicker($id)
	{
		...
		$this->lstFrom = new listbox($id.'_req_rc');
		$this->lstFrom->attributes['style'] = 'width:200px';
		$this->lstFrom->attributes['onclick'] = "unselect('".$id."_req_ru')";	
		$this->lstTo = new listbox($id.'_lstTo');
		$this->lstTo->attributes['style'] = 'width:200px';
		$this->lstTo->attributes['onclick'] = "unselect('".$id."_req_ru')";
    ...
    {		
		...
		{	
			...
			{
				$this->{$tag}->req_rc($vals[$i]['value'], $vals[$i+1]['value']);					
			}
		}
    }
}
