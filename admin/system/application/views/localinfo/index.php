    ...
    <tr>
        ...
        <td align="right" valign="middle"><?= $this->TVclass->language_dp('language', $this->session->userdata($session_keyword), "onChange='language_change(this.value,\"$session_keyword\")'") ?>
            <div class="buttons" style="float:right; margin-top:0px;">
                <a href="<?= base_url() ?>index.php/localinfo/add" class="btn btn-success" role="button">
...
<table border='0' cellspacing='0' cellpadding='3' width='99%' class="table table-bordered table-hover">
    ...
    if (count($localinfo) > 0) {
        foreach ($localinfo as $value) {
            ?>
        <tr class="active">
                <td valign="middle" align="center" width="25%"><img src="<?= $this->config->item('localinfo_icon_url') . $value->image ?>" title="<?= $value->name ?>" width="100" height="80"/></td>
                ...
                <td valign="middle" align="center" width="14%">
                    <a href="<?= base_url() ?>index.php/localinfo/edit/<?= $value->id ?>">
                        <span class="glyphicon glyphicon-edit"></span> Edit
                    </a>&nbsp;|&nbsp;
                    <a href="javascript:deleteconform('localinfo/delete','<?= $value->id ?>','')">
