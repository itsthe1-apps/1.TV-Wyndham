...
$opt_newsnpromo[''] = 'Select';
if (count($newsnpromo) > 0) {
    foreach ($newsnpromo as $li) {
        $opt_newsnpromo[$li->id] = $li->name;
    }
}
?>
    <table width="100%" border="0" cellpadding="5" cellspacing="0">
        <tr>
            ...
            <td width="30%"
                valign="top"><?= form_input('name', isset($edit_data['name']) ? $edit_data['name'] : $this->input->post('name'), 'maxlength=100') ?></td>
        ...
        <tr>
            ...
            <td valign="top">:</td>
            <td valign="top"><?= form_dropdown('newsnpromo', $opt_newsnpromo, isset($edit_data['newsnpromo']) ? $edit_data['newsnpromo'] : $this->input->post('newsnpromo')) ?></td>
            <td align="left" valign="top"><span id="error"><?= form_error('newsnpromo') ?></span></td>
        </tr>
        <tr>
            <td valign="top"><label for="image">Image <font class="image_size">(Width=<?= $image_width_menu ?>,
                        Height=<?= $image_height_menu ?>)</font></label><span class="star"> * </span></td>
            <td valign="top">:</td>
            <td valign="top">
                <?
                $st_img = isset($edit_data['image']) ? "<img src='" . $this->config->item('newsnpromo_icon_url') . $edit_data['image'] . "' align='right' width='100' height='80'>" : "";
