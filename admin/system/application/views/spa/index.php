            ...
            <tr>
                ...
                <td align="right" valign="middle">
                    <div class="main_control_btns" class="buttons" style="float:right; margin-top:0px;" > 
                        ...
                        <a href="<?= base_url() ?>index.php/spa/add_spa" class="btn btn-success" role="button"><span class="glyphicon glyphicon-plus-sign" style="padding-right:10px;"></span>Add Health Club</a>
                    </div>
                </td>
            </tr>
        </table>
        <div class="roundedcornr_box_main_tv" style="width:99%;">
            <div class="roundedcornr_content_main_tv">
                <table class="table table-bordered table-hover" border='0' cellspacing='0' cellpadding='0' width='99%'>
                    ...
                    <tbody>
                        <?php
                        if (count($spa_details) > 0) {
                            foreach ($spa_details as $key => $value) {
                                ?>
                                <tr>
                                    <td valign="left" ><?= $value->spa_type ?></td>            
                                    <td valign="left" ><?= $value->description ?></td> 
                                    <?php
                                    $image_names = explode("|", $value->spa_img_url);
                                    $html_img = '';
                                    foreach ($image_names as $img) {
                                        $html_img .= "<img  width='100'style='margin-bottom:5px;' src='" . $this->config->item('spa_icon_url') . $img . "'>" . "&nbsp;&nbsp;";
                                    }
                                    ?>
                                    <td valign="left"><?= $value->spa_img_url == "image" ? '-' : $html_img ?></td>
                                    <td width="10%" valign="left" align="center">
                                        <a href="<?= base_url() ?>index.php/spa/edit_spa/<?= $value->spa_id ?>">
                                            <span class="glyphicon glyphicon-edit"></span> Edit
                                        </a>&nbsp; | &nbsp;
                                        <a href="javascript:deleteconform('spa/delete','<?= $value->spa_id ?>','')">
