...
if ($task == 'edit') {
    ...
    foreach ($spa_data as $key => $value) {
      $spa_id = $value->spa_id;
      $spa_type = $value->spa_type;
      $spa_img_url = $value->spa_img_url;
      $spa_description = $value->description;
...
$data_type = array('name' => 'spa_type', 'id' => 'spa_type');
...
$data_description = array('name' => 'description', 'id' => 'description', 'rows' => 5, 'cols' => '40', 'value' => isset($spa_description) ? $spa_description : '', 'onkeypress' => 'check_length(this.form,this);');
...
$table.='<div class="device_details">';
$table.=' <div class="form-group">';
$table.='<label class="group_first_label" for="spa_type">Health Club Type <span class="star"> * </span></label>';
$table.='<label class="seperator_char">:</label>';
$table.= form_input(['name' => 'spa_type', 'id' => 'spa_type', 'class' => 'form-control', 'placeholder' => 'Health Club Type', 'value' => isset($spa_type) ? $spa_type : '']);
$table.="<span id='error'>".form_error('spa_type')."</span>";
$table.="</div>";
$table.="</div>";
...
isset($spa_img_url) ? $st_img = $spa_img_url : $st_img = "";
$img_validation = isset($spa_img_url) ? $img_error : '';

$image_names = explode("|",$st_img);
$html_img = '';
foreach ($image_names as $img) {
    $html_img .= "<img  width='100' src='".$this->config->item('spa_icon_url').$img."'>". "&nbsp;&nbsp;";
}
if ($st_img == '') {
    $html_img = '';
}
...
$table.=$html_img ."".form_hidden('spa_img_current', $st_img);
$table.="</div>";
$table.="<span id='error' valign='top' align='left'>'".isset($spa_img_url) ? $img_validation : ''."'</span>";
