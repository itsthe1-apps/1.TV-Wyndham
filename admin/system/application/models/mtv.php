...
class MTV extends Model{
	...
	function MTV(){
		...
		$this->channel			= $this->config->item($this->db->dbprefix.'channel');
		$this->itvtv_bygenre	= $this->config->item($this->db->dbprefix.'itvtv_bygenre');
	}
	
	function getAllTv($offset = 0, $row_count = 0, $var1=false, $var2=false){
		//$var1>0 ? $this->db->where('TVGenreID',$var1) : "";
		if($var1>0){;
			//$q = "id in(select TVCHannelID from itvtv_bygenre where TVGenreID=".$var1." group by TVChannelID)";
			//$this->db->where($q, NULL, FALSE);
...
//		die();
	...
	function getTv($id){
        ...
        function getTvByNumber($id){
		$data = array();		
		$options = array('number'=>$id);
		$Q = $this->db->getwhere($this->channel,$options,1);
		if ($Q->num_rows() > 0)
		{
			$data = $Q->row_array();
	...
	function UpdateTv(){
		...
		$data = array(
				...
				if ($image['file_name'])
				{
					$filename = TV_PATH."\\".$this->input->post('file_img_name');
	...
	function deleteTV($id,$id_g){
			...
			$filename 	= './icons/TV/'.$get_image['logo'];	
			if(file_exists($filename)) {
				unlink($filename);
			}
