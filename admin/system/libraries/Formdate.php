...
class FormDate
{
	 ...
	 * @return	string
	 */
	function selectYear($name, $selected_year)
	{
		// Variables
		$year = array();
		$year['cur'] = date('Y');
		...
		if (isset($this->year['options'])):
			...
			if($name=='from_year' || $name=='to_year')
			{
				...
				if($selected_year){ $selected = $selected_year; }else{ $selected = $this->year['selected']; }
			}
			
			return form_dropdown($name, $this->year['options'], isset($selected) ? $selected : "", "class='fields'");
	 ...
	 * @return	string
	 */
	function selectMonth($name, $selected_month)
	{
		// Variables
		$month = array();
		...
		if (isset($this->month['options'])):
			...
			if($name=='from_month' || $name=='to_month')
			{
				$this->month['options'][''] = 'MM';
				$selected = $selected_month;
			}else{
				if($selected_month!=""){ $selected = $selected_month; }else{ $selected = $this->month['selected']; }
			}
			
			return form_dropdown($name, $this->month['options'], $selected, "class='fields'");
	 ...
	 * @return	string
	 */
	function selectDay($name, $selected_day)
	{
		// Variables
		$day = array();
		$day['cur'] = date('j');
		...
		if (isset($this->day['options'])):
			...
			if($name=='from_day' || $name=='to_day')
			{
				$this->day['options'][''] = 'DD';
				$selected = $selected_day;
			}else{
				if($selected_day!=""){ $selected = $selected_day; }else{ $selected = $this->day['selected']; }
			}
			return form_dropdown($name, $this->day['options'], $selected, "class='fields'");
	 ...
	 * @return	string
	 */
	function selectHour($name, $selected_hour)
	{
		// Variables
		$hour = array();
		...
		if (isset($this->hour['options'])):
			...
			if($selected_hour!=""){ $selected = $selected_hour; }else{ $selected = $this->hour['selected']; }
			
			return form_dropdown($name, $this->hour['options'], $selected, "class='fields'");
	 ...
	 * @return	string
	 */
	function selectMinute($name, $selected_minute)
	{
		// Variables
		$minute = array();
		$minute['cur'] = date('i');
		...
		if (isset($this->minute['options'])):
			...
			if($selected_minute!=""){ $selected = $selected_minute; }else{ $selected = $this->minute['selected']; }
			
			return form_dropdown($name, $this->minute['options'], $selected, "class='fields'");
	 ...
	 * @return	string
	 */
	function selectSecond($name, $selected_second)
	{
		// Variables
		$second = array();
		$second['cur'] = date('s');
		...
		if (isset($this->second['options'])):
			...
			if($selected_second!=""){ $selected = $selected_second; }else{ $selected = $this->second['selected']; }
		
			return form_dropdown($name, $this->second['options'], $selected, "class='fields'");
	...
	function selectMeridian()
	{
		...
		// Return form dropdown
		return form_dropdown($this->config['prefix'].$this->meridian['name'], $this->meridian['options'], $this->meridian['selected'], "class='fields'");
	}
